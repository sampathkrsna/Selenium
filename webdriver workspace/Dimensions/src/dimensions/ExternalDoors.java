...
public class ExternalDoors {
	...
	public void beforeMethod(String url) throws InterruptedException, IOException {
		...
		File file = new File(project_path + "/XSLT_Reports/"+systemDate+"/errors/external.txt");
	...
	public void fullpanel(String url) throws InterruptedException, IOException {
		...
		shuffleArray(array);
		checkExternalDoors(array, url,(max-min+1));
		checkedFlow(array);
	}
	...
	public void checkExternalDoors(int[] array, String url,int iterations) throws InterruptedException, IOException{
		for(int i=0 ; i<iterations ; i++){
		    ...
		    checkDimensionsPage(url, "externalDoorSelection", Integer.toString(width_feet),  Integer.toString(width_inch), "6", "5");
		}
	}
